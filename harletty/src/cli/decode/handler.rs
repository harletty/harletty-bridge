...
use damf::{BedInstance, Configuration, Event, SourceCodec};
...
impl AudioFormatHandler {
    ...
    fn rename_and_recreate_caf_writer(
        ...
        let caf_writer = {
            ...
            damf::caf::CAFWriter::from_parsed_info(BufWriter::new(file), file_info)?
        };
...
impl AudioDataConverter {
    fn convert_caf_bytes_to_samples(buffer: &[u8], endianness: damf::caf::Endianness) -> Vec<i32> {
        ...
        for chunk in buffer.chunks_exact(BYTES_PER_SAMPLE) {
            let sample = match endianness {
                damf::caf::Endianness::BigEndian => {
                    i32::from_be_bytes([0, chunk[0], chunk[1], chunk[2]]) >> 8
                }
                damf::caf::Endianness::LittleEndian => {
...
pub struct DecodeHandler {
    ...
    pub current_segment_base_path: Option<PathBuf>,
...
impl Default for DecodeHandler {
    fn default() -> Self {
        Self {
            ...
            current_segment_base_path: None,
...
impl DecodeHandler {
            ...
            if !was_atmos {
                if let Some(base_path) = base_path {
                    ...
                    let effective_base_path = if self.is_segmented {
                        if let Some(ref segmented_base_path) = self.current_segment_base_path {
                            segmented_base_path.clone()
                        } else if let Some(ref current_path) = self.current_audio_path {
    ...
    pub fn handle_stream_restart(
        ...
        if let Some(base_path) = base_path {
            ...
            self.current_segment_base_path = Some(segmented_base_path.clone());
